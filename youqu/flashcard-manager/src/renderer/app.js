...
async function renderDecks() {
  ...
  totalCardEl.textContent = '共 ' + state.decks.length + ' 个卡组 · ' + total + ' 张卡片';
}
...
async function renderDeckDetail() {
  ...
  mainEl.innerHTML =
    ...
    '</div>' +
    '<div class="stat-grid">' +
      statCard('今日待复习', dueCount, 'due') +
      statCard('未学新卡', state.stats.fresh, 'fresh') +
      statCard('已学习中', state.stats.learned, '') +
      statCard('卡片总数', state.stats.total, '') +
    '</div>' +
    '<div class="action-bar">' +
      '<button class="btn-primary" id="btn-review" ' + (state.stats.total === 0 ? 'disabled' : '') + '>' +
...
function cardRowEl(c) {
  ...
  const due = c.due && c.due > Date.now() ? fmtDate(c.due) + ' 复习' : '待复习';
  div.innerHTML =
    '<div class="card-row-head">' +
      ...
      '<div class="card-actions">' +
        '<button class="icon-btn" title="编辑" data-act="edit">' +
          '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 20h4L18.5 9.5a2.12 2.12 0 00-3-3L5 17v3z"/></svg>' +
        '</button>' +
        '<button class="icon-btn danger" title="删除" data-act="del">' +
          '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 7h14M10 7V5h4v2M6 7l1 13h10l1-13"/></svg>' +
        '</button>' +
      '</div>' +
    '</div>' +
    '<div class="card-back">' + escapeHtml(c.back) + '</div>' +
    '<div class="card-meta">' +
      '<span class="meta-item">已复习 ' + (c.reps || 0) + ' 次</span>' +
      '<span class="meta-item">·</span>' +
      '<span class="meta-item">' + due + '</span>' +
      '<span class="meta-item">·</span>' +
      '<span class="meta-item">易度 ' + (c.ef || 2.5).toFixed(2) + '</span>' +
    '</div>';
...
function showReviewComplete() {
  ...
  overlay.querySelector('.review-stage').innerHTML =
    '<div class="review-complete">' +
      ...
      '<button class="btn-primary" id="rv-back" style="margin-top:8px;">返回卡组</button>' +
    '</div>';
