...
| [📝 便签管家](./sticky-notes-manager) | 本地便签桌面应用：快速记录、分类管理、置顶标记、全文搜索、颜色标签、导入导出、全局快捷键、托盘常驻，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
<!-- 新项目在这里追加一行 -->
...
| [⚖️ 换算管家](./unit-converter-manager) | 本地离线多类别单位换算桌面工具：15 大类（长度/面积/体积/质量/温度/速度/时间/数据/压力/角度/能量/功率/油耗/频率/进制）、实时批量换算、公制/英制/市制、收藏与历史、纯本地隐私 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
