...
- **明细管理** — 全部/支出/收入/转账筛选，支持编辑、删除（二次确认防误操作），支持搜索备注和分类名
- **统计视图** — 收支分类占比条形图，直观对比各分类金额
- **预算管理** — 设置月度预算，环形进度环实时展示使用率，超支预警
- **账户余额** — 🆕 v1.1.0 新增：查看各账户余额、总资产、总负债、净资产，资产负债分布图，账户统计汇总
- **账户转账** — 🆕 v1.2.0 新增：账户间资金转账，自动调整两端账户余额，不计入收支统计与预算，资金流动清晰不扭曲月度报表
...
### v1.2.0（2026-07-14）
- 🆕 **新增「账户转账」功能** —— 在两个账户之间转移资金（如银行卡→支付宝），自动调整两端账户余额，不计入收支统计与预算，资金流动清晰不扭曲月度报表
- 🆕 记一笔弹窗新增「转账」类型，支持转出/转入账户选择（含相同账户联动校验）
- 🆕 明细视图新增「转账」筛选 Tab，转账记录以蓝灰专属样式区分于收支
- 🐛 **修复统计 bug**：修复概览/统计/趋势/日历视图中转账被误算为支出的逻辑漏洞（原 `else expense` 改为 `else if expense`）
- 🎨 转账明细采用 🔄 图标 + 蓝色中性金额 + 渐变背景，视觉上与收支明确区分
- ✅ 单元测试从 93 项增至 115 项，新增 22 项转账全流程测试（创建/校验/筛选/统计排除/余额计算/更新/CSV/删除）
...
覆盖日期工具、交易存储、预算、统计、导入导出、示例数据、清空数据、账户余额、账户转账等 115 项测试。
