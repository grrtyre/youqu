...
const convertCore = require('./core/convert-core.js');
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'renderer', 'index.html'), SCREENSHOT_MODE ? { hash: 'convert' } : undefined);
...
ipcMain.handle('convert:run', async (event, payload) => {
  try {
    const value = parseFloat(payload && payload.value);
    if (!isFinite(value)) throw new Error('数值无效');
    const fromUnit = String((payload && payload.fromUnit) || '');
    const toUnit = String((payload && payload.toUnit) || '');
    const category = String((payload && payload.category) || '');
    const result = convertCore.convert(value, fromUnit, toUnit, category);
    return { ok: true, value: result, formatted: convertCore.formatResult(result) };
  } catch (err) {
    return { ok: false, error: err.message || String(err) };
  }
});

ipcMain.handle('convert:list', async () => {
  return { ok: true, categories: convertCore.listCategories() };
});

ipcMain.handle('convert:common', async (event, payload) => {
  try {
    const category = String((payload && payload.category) || '');
    const items = convertCore.getCommonConversions(category);
