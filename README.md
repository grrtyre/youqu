...
| [🎨 调色板管理器](./palette-manager) | 基于色彩理论的桌面配色方案生成器：6 种和谐模式（类比/互补/三元/四元/分裂互补/同色系）、一键随机、锁定微调、图片取色（K-means）、WCAG 对比度检查、CSS/SCSS/JSON/Tailwind 多格式导出、PNG 预览、本地收藏、托盘常驻、全局快捷键 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [📊 系统监控器](./system-monitor) | 实时系统资源监控仪表盘：CPU/内存/磁盘/网络/进程，圆环仪表盘+历史曲线图，苹果白风格，本地运行零数据外传 | Node.js |
| [💡 提示词库管家](./prompt-manager) | AI 时代的提示词管理利器：分类、标签、收藏、全文搜索、`{{变量}}`填写、一键复制、使用统计、JSON 导入导出，苹果白高端风格，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [🖼️ 图片压缩器](./image-compressor) | 苹果白风格图片批量压缩工具：拖拽即用、质量滑块+三档预设、JPG/PNG/WebP 格式互转、实时节省率统计、本地处理隐私安全 | Electron + Sharp | ✅ 可用 v1.0.0 |
| [🔐 验证器管家](./authenticator-manager) | 本地优先的两步验证器桌面应用：TOTP/RFC 6238 自研算法、safeStorage 加密存储、全局热键唤起、失焦自动隐藏、系统托盘常驻、单实例锁、AES-256-GCM 加密备份、otpauth URI 解析、倒计时环、复制后自动隐藏，苹果白高端风格 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [📚 稍后阅读管家](./reading-list-manager) | 稍后阅读桌面应用：URL 快速收藏、剪贴板全局热键 Ctrl+Shift+L、状态管理（未读/阅读中/已读/已归档）、标签分类、即时搜索、JSON 导入导出、系统托盘常驻，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [🎵 音乐](./music-player) | 苹果白高端风格本地音乐播放器：简洁纯粹、多格式支持（MP3/WAV/FLAC/OGG/M4A/AAC）、拖拽导入、随机/循环模式、动态渐变封面、列表持久化、快捷键，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [🎙️ 语音备忘录管家](./voice-memo-manager) | 本地优先的语音备忘录桌面应用：实时录音+波形可视化、一键播放、重命名、文件夹定位、删除、搜索过滤、系统托盘、快捷键，苹果白高端风格，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [🏠 房贷计算器](./mortgage-manager) | 房贷计算器：商业贷款/公积金/组合贷款/提前还款测算，等额本息/等额本金，月供明细表+饼图+折线图，CSV导出，苹果白高端风格，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [🔑 许可证管理器](./license-manager) | 本地加密软件许可证管理器：AES-256-GCM + PBKDF2 密钥派生、主密码保护、到期提醒、分类筛选、全文搜索、加密备份导入导出、5 分钟自动锁定，苹果白高端风格，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
<!-- 新项目在这里追加一行 -->
...
| [快速翻译器](./quick-translate) | 托盘驻留翻译工具，全局快捷键，双引擎，20 种语言 | Electron | v1.0.0 |
| [🖼️ 壁纸管家](./wallpaper-manager) | 苹果白高端风格桌面壁纸管理：本地壁纸库 + 必应每日壁纸下载 + 定时切换 + 收藏管理 + 系统托盘 + 单实例锁，纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [看板管家](./kanban-manager) | 苹果白风格本地优先看板任务管理工具，支持拖拽、标签、截止日期、优先级、归档与导入导出 | Electron | v1.0.0 |
| [💸 分账助手](./expense-splitter) | 群组 AA 账单分账：合租/聚餐/旅行多人共同消费记录、四种分摊方式（均摊/按份数/百分比/自定义）、智能余额计算、贪心算法最少笔数结算建议、结算追踪、本地持久化 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
