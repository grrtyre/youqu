...
| [📝 文本管家](./text-manager) | Windows 批量文本处理工具：替换/分割/提取/大小写/去重/行处理/统计，苹果白风格、纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
...
| [📋 代码片段管家](./snippet-manager) | 本地优先的代码片段管理桌面应用：自研语法高亮引擎（20+语言）、全文搜索、标签分类、收藏置顶、导入导出、全局快捷键、托盘常驻 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [💽 磁盘管家](./disk-manager) | 磁盘空间可视化分析器：Squarified Treemap、分层钻取、大文件排行、类型分布、纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
<!-- 新项目在这里追加一行 -->
