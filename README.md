...
| [🧮 计算器管家](./calculator-manager) | 苹果白高端风格桌面计算器：科学/程序员/基础三模式、自研表达式引擎、变量定义、历史记录、纯本地隐私优先 | Electron + 原生 JS | ✅ 可用 v1.0.0 |
| [😀 表情管家](./emoji-manager) | 苹果白风格桌面 emoji 管理器：600+ 表情/50+ 颜文字/特殊符号、一键复制、本地收藏、历史记录、全局快捷键 Ctrl+Shift+E | Electron + 原生 JS | ✅ 可用 v1.0.0 |
<!-- 新项目在这里追加一行 -->
