...
- 🔍 **放大镜 + 取色** — 跟随鼠标的圆形放大镜，6 倍像素级放大；HUD 实时显示光标像素颜色（HEX/RGB），点击一键复制色值
- ✏️ **自由标注** — 在屏幕上自由涂画，标记重点区域
- 💾 **一键保存** — 将当前测量画面保存为 PNG 图片，方便分享
- 🖥 **多显示器支持** — 自动在光标所在的显示器上唤起并截屏，多屏环境随心测量
- 🪟 **跨平台标题栏** — 无边框窗口 + 功能化红黄绿小圆点，Windows 不再出现双重标题栏
...
| 平台 | 下载链接 | 说明 |
| --- | --- | --- |
| Windows x64 | [屏幕标尺管家 Setup 1.1.0.exe](https://github.com/grrtyre/youqu/releases/download/screen-ruler-v1.1.0/Setup.1.1.0.exe) | 安装版，支持自定义安装路径 |
| Windows x64 | [屏幕标尺管家 1.1.0.exe](https://github.com/grrtyre/youqu/releases/download/screen-ruler-v1.1.0/1.1.0.exe) | 便携版，双击即用 |
