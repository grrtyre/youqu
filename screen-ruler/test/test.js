...
console.log('\n[9] findDisplayAt（多显示器选屏）');
const displays = [
  { id: 1, bounds: { x: 0, y: 0, width: 1920, height: 1080 } },
  { id: 2, bounds: { x: 1920, y: 0, width: 2560, height: 1440 } },
  { id: 3, bounds: { x: -1366, y: 0, width: 1366, height: 768 } }
];
assert(M.findDisplayAt(displays, { x: 100, y: 200 }).id === 1, '主显示器内的点→返回主显示器');
assert(M.findDisplayAt(displays, { x: 2000, y: 500 }).id === 2, '右侧显示器内的点→返回右侧');
assert(M.findDisplayAt(displays, { x: -500, y: 300 }).id === 3, '左侧显示器内的点→返回左侧');
assert(M.findDisplayAt(displays, { x: 1920, y: 0 }).id === 2, '边界 x=1920 属于右侧显示器（左闭右开）');
assert(M.findDisplayAt(displays, { x: 1919, y: 0 }).id === 1, 'x=1919 仍属主显示器');
assert(M.findDisplayAt(displays, { x: 5000, y: 5000 }) === null, '所有显示器外的点→返回 null');
assert(M.findDisplayAt(null, { x: 0, y: 0 }) === null, 'displays 非数组→返回 null');
assert(M.findDisplayAt(displays, null) === null, 'point 为空→返回 null');
