...
const fs = require('fs');
const os = require('os');
const { mergeConfig, validateConfig, resolveVariables, formatTime, isInSchedule, pickPhysicalIP, DEFAULT_CONFIG, TEMPLATES } = require('./core/watermark-core');
...
function getLocalIP() {
  try {
    return pickPhysicalIP(os.networkInterfaces());
  } catch (_) {}
  return '127.0.0.1';
}
...
function shouldShowWatermark() {
  ...
  if (currentConfig.scheduleEnabled) {
    return isInSchedule(new Date(), currentConfig);
  }
  return true;
}

// 应用水印显示状态（总开关 + 定时共同决定）
// 由 refreshWatermark 与调度定时器统一调用
function applyWatermarkState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    try { mainWindow.webContents.send('config:updated', currentConfig); } catch (_) {}
  }
  if (tray && tray.updateMenu) tray.updateMenu();
}

// 刷新水印显示（配置变化时）—— 统一走调度感知的状态应用
function refreshWatermark() {
  applyWatermarkState();
}

// 定时水印调度定时器（每 30 秒检查是否在区间内，自动开关覆盖层）
let scheduleTimer = null;
function startScheduleTimer() {
  stopScheduleTimer();
  scheduleTimer = setInterval(() => {
    if (!currentConfig || !currentConfig.enabled) return;
    if (!currentConfig.scheduleEnabled) return;
    applyWatermarkState();
  }, 30 * 1000);
}
...
function createTray() {
  ...
  function updateTrayMenu() {
    ...
    const active = shouldShowWatermark();
    let statusLabel;
    if (!enabled) {
      statusLabel = '○ 水印已关闭';
    } else if (currentConfig.scheduleEnabled) {
      statusLabel = active ? '🕑 水印运行中（定时）' : '⏸ 定时未到·已隐藏';
    } else {
      statusLabel = '✓ 水印运行中';
    }
...
// 水印覆盖层首次渲染完成信号（用于验证 overlay 真正加载并绘制）
let overlayRendered = false;
ipcMain.on('overlay:rendered', () => {
  if (overlayRendered) return;
  overlayRendered = true;
  try {
    fs.writeFileSync(path.join(os.tmpdir(), 'watermark-overlay-rendered.log'),
      new Date().toISOString() + ' overlay rendered\n', 'utf8');
  } catch (_) {}
});
