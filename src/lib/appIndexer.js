...
class AppIndexer {
  ...
  isUnwanted(name) {
    ...
    // D1: 用词边界匹配，避免误伤 HelpDesk / HelpScout / Eclipse Help 等正常应用
    // 仅过滤独立的 "uninstall"/"卸载"/"help"/"readme"/"license"/"访问网站"/"website"
    const keywords = ['uninstall', '卸载', 'readme', 'license', '访问网站', 'website'];
    if (keywords.some(k => n.includes(k))) return true;
    // help 单独匹配词边界（前后为非字母数字或字符串首尾）
    if (/(^|[^a-z0-9])help([^a-z0-9]|$)/i.test(name)) return true;
    return false;
  }
}
