...
// CSV 转义测试（RFC 4180）
const escStore = new TimeStore(tmpFile());
const escProj = escStore.createProject({ name: '项目A, 含"引号"', color: '#007aff' });
escStore.addRecord({ projectId: escProj.id, start: Date.now() - 60000, end: Date.now(), note: '备注, 也含逗号' });
const escCsv = escStore.exportCSV();
// 表头字段应被双引号包裹
assert(escCsv.startsWith('\uFEFF"日期","项目"'), 'CSV 表头应按 RFC 4180 双引号包裹，得到: ' + escCsv.slice(0, 40));
// 含特殊字符的数据行：用状态机解析整行
const escLines = escCsv.split('\n');
const dataLine = escLines.find((l) => l.includes('项目A')) || '';
...
const parsedRow = parseCsvRow(dataLine);
assert(parsedRow.length === 6, 'CSV 含特殊字符的行应有 6 个字段（逗号不应拆列），得到: ' + parsedRow.length);
assert(parsedRow[1] === '项目A, 含"引号"', 'CSV 解析后项目名应还原原值（逗号+引号），得到: ' + JSON.stringify(parsedRow[1]));
assert(parsedRow[5] === '备注, 也含逗号', 'CSV 解析后备注应还原原值（含逗号），得到: ' + JSON.stringify(parsedRow[5]));
try { fs.unlinkSync(escStore.filePath); } catch (e) {}

// importJSON 校验测试
const badStore = new TimeStore(tmpFile());
badStore.createProject({ name: '原有项目', color: '#007aff' });
let importThrew = false;
try { badStore.importJSON('{"foo":"bar"}'); } catch (e) { importThrew = true; }
assert(importThrew, 'importJSON 对缺 projects/records 数组的文件应抛错');
assert(badStore.listProjects().length === 1, 'importJSON 失败不应覆盖原有数据');
let importThrew2 = false;
try { badStore.importJSON('not json'); } catch (e) { importThrew2 = true; }
assert(importThrew2, 'importJSON 对非法 JSON 应抛错');
try { fs.unlinkSync(badStore.filePath); } catch (e) {}
